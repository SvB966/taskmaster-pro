...
import { PlusCircle, Save, Trash2, X, Clock, AlertCircle, CheckSquare, Square, Plus } from 'lucide-react';
...
export const TaskPanel: React.FC<TaskPanelProps> = ({ selectedDate, tasks, onAdd, onUpdate, onDelete }) => {
  // Format YYYY-MM-DD
             ...
             <div className="bg-white dark:bg-slate-700/50 p-2 rounded border border-slate-200 dark:border-slate-600 transition-colors">
               ...
               <div className="space-y-2 mb-2">
                 {formData.subtasks.map(st => (
                   <div key={st.id} className="flex items-center justify-between bg-slate-50 dark:bg-slate-700 px-2 py-1 rounded border border-slate-100 dark:border-slate-600">
                     <span className="text-sm truncate text-slate-700 dark:text-slate-200">{st.title}</span>
                     <button 
                       type="button" 
             ...
             {dayTasks.sort((a,b) => a.startTime.localeCompare(b.startTime)).map(task => {
                ...
                return (
                 ...
                 >
                   ...
                   <h4 className="text-base font-semibold text-slate-800 dark:text-slate-100 leading-tight mb-1">{task.title}</h4>
